...
class SPWaterProgressIndicatorView: UIView {
    ...
    override init(frame: CGRect) {
        ...
        let progressPath = UIBezierPath(rect: self.bounds)
...
//        self.layer.addSublayer(self.textLayer)
    ...
    override func draw(_ rect: CGRect) {
        ...
        while  x < (width + self.density) {
            ...
            let y = scaling * maxAmplitude * self.amplitude * sin(CGFloat(2.0 * .pi) * (x / width) * self.frequency + self.phase) + self.bounds.height * CGFloat(100 - self.completionInPercent) / CGFloat(100)
